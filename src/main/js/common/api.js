...
export function findProjects() {
  return getJSON("/api/projects/search").then(function(response) {
    return response.components.map(node=>{
      let p={"name":node["name"],"key":node["key"]};
      return p;
    });
  });
}

export function findProjectJunitResult(project) {
  return getJSON("/api/measures/search_history", {
    ...
    let map={}
    let data=responseMetrics.measures
    if(data){
      data.forEach(n=>{
        if(n.history){
          n.history.forEach((his)=>{
            let date=his.date
            if(date){
              date=his.date.substr(0,date.indexOf("+"))
              date=date.replaceAll("T"," ")
            }
            if(!map[date]){
              map[date]={}
            }
            map[date][n.metric]=his.value||0
          })
        }
      })
    }
